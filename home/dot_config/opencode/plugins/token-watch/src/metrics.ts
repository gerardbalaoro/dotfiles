...
type SessionWithRollup = Omit<Session, "cost" | "tokens"> & {
  cost?: number;
  tokens?: {
    total?: number;
    input?: number;
...
export class Metrics {
  ...
  static fromSessionRollup(session: Session): Metrics | undefined {
    ...
    const hasRollup = (data: SessionWithRollup) =>
    ...
    const data = session as SessionWithRollup;
    if (!hasRollup(data)) return undefined;
  ...
  static async fromSession(api: TuiPluginApi, session: Session) {
    ...
    const data = local
      ? (session as SessionWithRollup)
      : await api.client.session
          .get({ sessionID: session.id })
          .then((r) => r.data as SessionWithRollup | undefined)
          .catch(() => undefined);

    return data ? (Metrics.fromSessionRollup(data as Session) ?? new Metrics()) : new Metrics();
  }
